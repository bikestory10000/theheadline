...
week = sys.argv[1] if len(sys.argv) > 1 else "W37"
files = sys.argv[2:] or sorted(pathlib.Path("data").glob("NEWS_IDIOM_00*.json"))[:5]
...
rows = "".join(f"""<div class="box" style="display:grid;grid-template-columns:100px 1fr 1fr;gap:20px;align-items:center;padding:14px 28px">
...
c1 = f"""
<div class="si" style="padding-top:104px;padding-bottom:72px">
  <div class="chip">WEEKLY FIVE · {e(week)}</div>
  <div class="gw" style="font-size:60px;line-height:1.25;margin-bottom:10px">이번 주 표현 5개, 한 장으로</div>
  ...
  <div class="cta"><p>다음 장에서 5문항 복습<small>정답은 캡션에</small></p></div>
</div>
"""
...
<div class="si" style="padding-top:104px;padding-bottom:72px">
  ...
  <div class="gw" style="font-size:52px;line-height:1.25;margin-bottom:22px">빈칸 5개, 30초 안에</div>
...
"""
alts = {1: f"주간 치트시트 {week}. 표현 5개와 뜻, 시험 환언", 2: f"주간 복습 퀴즈 {week}. 빈칸 5문항"}
for n, body, glow in [(1, c1, (300,260)), (2, c2, (540,300))]:
    p = OUT / f"WEEKLY_FIVE_{week}_card_0{n}.html"
    p.write_text(shell(n, 2, body, alts[n], "weekly", f"WEEKLY_FIVE_{week}", glow), encoding="utf-8"); print("wrote", p)
